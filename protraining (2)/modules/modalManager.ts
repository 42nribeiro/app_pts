// modules/modalManager.ts
import * as DOM from './domElements';
import { state } from './config';
import { Plan, Exercise, MasterExercises, AvaliacaoData, ClientStats } from './interfaces';
...
// Re-exporting or using a direct call to planManager's adicionarExercicioComDados
...
export function handleExerciseFocus(buttonElement: HTMLButtonElement) { 
    const inputElement = buttonElement.previousElementSibling as HTMLInputElement;
    // Ensure the element exists and is not inside a readonly/disabled plan item
    if (!inputElement || inputElement.closest('.plan-readonly') || inputElement.closest('.item-readonly')) return;
    showExerciseModal(inputElement);
}

export function showExerciseModal(targetInputElement: HTMLInputElement) { 
    if (typeof state.MASTER_EXERCISE_DATA === 'undefined') { 
        showCustomAlert("Lista de exercícios mestre não carregada.", true); return; 
    }
    state.currentExerciseInputTarget = targetInputElement;
    state.fullModalExerciseList = []; // Reset
    const exerciseContainer = targetInputElement.closest('.exercise-container');
    const grupoMuscularSelect = exerciseContainer?.querySelector('.grupoMuscular') as HTMLSelectElement;
    ...
    if (!selectedMuscleGroup) {
        ...
        state.currentExerciseInputTarget = null; // Clear target if no group selected
        return;
    }
    ...
    if(DOM.modalSearchInput) {
        DOM.modalSearchInput.oninput = () => { // Re-assign oninput
            if (!DOM.modalSearchInput) return;
    ...
    // Use the main alertOverlay for the exercise modal's backdrop
    if (DOM.alertOverlay) {
        DOM.alertOverlay.style.display = 'block';
        DOM.alertOverlay.classList.add('show'); // Make it visible
    }
...
export function hideExerciseModal() { 
    // Hide the main alertOverlay which acts as backdrop for exercise modal
    if (DOM.alertOverlay) {
        DOM.alertOverlay.classList.remove('show');
        // Set display to none after transition (if any, or directly)
        // Check if another alert is active before hiding overlay completely
        if (!DOM.alertDiv?.classList.contains('show')) {
             setTimeout(() => { if(DOM.alertOverlay) DOM.alertOverlay.style.display = 'none'; }, 250); // Match potential transition
        }
    }
    if (DOM.exerciseModal) DOM.exerciseModal.style.display = 'none'; 
    if (DOM.modalListContainer) DOM.modalListContainer.innerHTML = ''; 
    state.currentExerciseInputTarget = null; 
    state.fullModalExerciseList = []; 
    if (DOM.modalSearchInput) DOM.modalSearchInput.oninput = null; // Clean up listener
}

export function populateModalList(list: string[], searchTerm = '') {
    if (!DOM.modalListContainer) return;
...
export function selectExerciseFromModal(selectedExerciseText: string) {
...
export async function addNewExerciseToList(newExerciseName: string) {
    if (!state.currentExerciseInputTarget) return;
    const trimmedName = newExerciseName.trim();
    if (!trimmedName) { showCustomAlert("Nome do exercício não pode ser vazio.", true); return; }

    const exerciseContainer = state.currentExerciseInputTarget.closest('.exercise-container');
    const grupoMuscularSelect = exerciseContainer?.querySelector('.grupoMuscular') as HTMLSelectElement;
    const selectedMuscleGroup = grupoMuscularSelect?.value;

    if (!selectedMuscleGroup) { showCustomAlert("Grupo Muscular não selecionado.", true); return; }
    if (typeof state.MASTER_EXERCISE_DATA === 'undefined') { showCustomAlert("Lista de exercícios mestre não carregada.", true); return; }
    ...
    if (!state.MASTER_EXERCISE_DATA[selectedMuscleGroup]) state.MASTER_EXERCISE_DATA[selectedMuscleGroup] = [];
    ...
    if (state.currentExerciseInputTarget) state.currentExerciseInputTarget.dataset.exerciseList = JSON.stringify(state.MASTER_EXERCISE_DATA[selectedMuscleGroup]);
    selectExerciseFromModal(trimmedName); 
...
export function abrirModalAvaliacao(idHTML: number) {
    const plano = state.planos.find(pl => pl.contadorPlano === idHTML);
    if (!plano) { showCustomAlert("Plano não encontrado para registrar avaliação.", true); return; }

    if(DOM.avaliacaoForm) DOM.avaliacaoForm.reset();
    if(DOM.avaliacaoPlanUuidInput) DOM.avaliacaoPlanUuidInput.value = plano.planUuid;
    if(DOM.avaliacaoContadorPlanoInput) DOM.avaliacaoContadorPlanoInput.value = String(idHTML);

    if (plano.avaliacao) {
        const aval = plano.avaliacao;
        if(DOM.avalPesoInput) DOM.avalPesoInput.value = String(aval.peso ?? '');
        if(DOM.avalAlturaInput) DOM.avalAlturaInput.value = String(aval.altura ?? '');
        if(DOM.avalMassaMuscularInput) DOM.avalMassaMuscularInput.value = String(aval.massaMuscular ?? '');
        if(DOM.avalMassaGordaInput) DOM.avalMassaGordaInput.value = String(aval.massaGorda ?? '');
        if(DOM.avalGorduraVisceralInput) DOM.avalGorduraVisceralInput.value = String(aval.gorduraVisceral ?? '');
        if(DOM.avalIdadeBiologicaInput) DOM.avalIdadeBiologicaInput.value = String(aval.idadeBiologica ?? '');
        if(DOM.avalMetabolismoBasalInput) DOM.avalMetabolismoBasalInput.value = String(aval.metabolismoBasal ?? '');
        if(DOM.avalMassaOsseaInput) DOM.avalMassaOsseaInput.value = String(aval.massaOssea ?? '');
        if(DOM.avalH2OInput) DOM.avalH2OInput.value = String(aval.h2o ?? '');
        if(DOM.avalPressaoArterialInput) DOM.avalPressaoArterialInput.value = aval.pressaoArterial || '';
        if(DOM.avalPerimetroAbdominalInput) DOM.avalPerimetroAbdominalInput.value = String(aval.perimetroAbdominal ?? '');
        if(DOM.avalObservacoesInput) DOM.avalObservacoesInput.value = aval.observacoes || '';
    }
    if (DOM.avaliacaoModalOverlay) DOM.avaliacaoModalOverlay.style.display = 'block';
    if (DOM.avaliacaoModal) DOM.avaliacaoModal.style.display = 'flex';
    setTimeout(() => {
        if (DOM.avaliacaoModal) DOM.avaliacaoModal.classList.add('show');
        if (DOM.avaliacaoModalOverlay) DOM.avaliacaoModalOverlay.classList.add('show');
        DOM.avalPesoInput?.focus();
    }, 50);
}
export function fecharModalAvaliacao() { 
    if (DOM.avaliacaoModal) DOM.avaliacaoModal.classList.remove('show'); 
    if (DOM.avaliacaoModalOverlay) DOM.avaliacaoModalOverlay.classList.remove('show'); 
    setTimeout(() => { 
        if (DOM.avaliacaoModal) DOM.avaliacaoModal.style.display = 'none'; 
        if (DOM.avaliacaoModalOverlay) DOM.avaliacaoModalOverlay.style.display = 'none'; 
    }, 250); 
}
export function salvarAvaliacao() {
    if (!DOM.avaliacaoContadorPlanoInput || !DOM.avaliacaoPlanUuidInput) {
        showCustomAlert("Erro: Elementos do formulário de avaliação não encontrados.", true); return;
    }
    ...
    if (!plano) { showCustomAlert("Erro: Plano não encontrado para salvar avaliação.", true); return; }

    const getNumOrNull = (el: HTMLInputElement | null, isFloat = false): number | null => {
        const val = el?.value.trim();
        if (val === '' || val === undefined || val === null) return null;
        const num = isFloat ? parseFloat(val) : parseInt(val, 10);
        return isNaN(num) ? null : num;
    };
    const getStringOrNull = (el: HTMLInputElement | HTMLTextAreaElement | null): string | null => el?.value.trim() || null;
    ...
    salvarPlano(idHTML); // This is planManager.salvarPlano
...
export async function abrirModalEstatisticas() {
    ...
    if (!nomeFiltroVal) { showCustomAlert("Por favor, insira um nome de cliente no filtro para buscar estatísticas.", true); return; }
    ...
    try {
        ...
        const data: ClientStats = await fetchApi(`/api/stats/cliente?${params.toString()}`, {}, 'abrirModalEstatisticas');
        
        // Dynamically import chartManager to avoid circular dependencies at module load time
        ...
        if (DOM.estatisticasModal) DOM.estatisticasModal.style.display = 'none';
        if (DOM.estatisticasModalOverlay) DOM.estatisticasModalOverlay.style.display = 'none';
    } finally {
        enableActionButtons();
    }
}
export function fecharModalEstatisticas() { 
    if (DOM.estatisticasModal) DOM.estatisticasModal.classList.remove('show'); 
    if (DOM.estatisticasModalOverlay) DOM.estatisticasModalOverlay.classList.remove('show'); 
    setTimeout(() => { 
        if (DOM.estatisticasModal) DOM.estatisticasModal.style.display = 'none'; 
        if (DOM.estatisticasModalOverlay) DOM.estatisticasModalOverlay.style.display = 'none'; 
    }, 250); 
    ...
    for (const key in state.activeCharts) { delete state.activeCharts[key]; }
}
